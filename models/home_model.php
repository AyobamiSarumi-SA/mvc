...
class home_model extends db{
   ...
   public function __construct(){
       ...
       if(empty($_POST['firstname']) || empty($_POST['lastname']) || empty($_POST['email']) || empty($_POST['phone']) || empty($_POST['address'])){
        ...
        $checkemail = $this->_select('users', '*', array('email' => $_POST['email']));

        $checkphone = $this->_select('users', '*', array('phone' => $_POST['phone']));
        ...
        $this->_insert('users', array('firstname' => strip_tags($_POST['firstname']), 'lastname' => strip_tags($_POST['lastname']), 'email' => $_POST['email'], 'dob' => $_POST['dob'], 'phone' => $_POST['phone'], 'address' => $_POST['address'], 'password' => password_hash($_POST['password'], PASSWORD_DEFAULT), 'token' => password_hash("IloveToTakeProgramClassButItBoresMeSometimes", PASSWORD_DEFAULT)));
